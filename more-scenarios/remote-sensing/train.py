...
from models.model.deeplab_v3_plus_resnest50_ca import DeepV3Plus_CA
...
from models.nets.deeplabv3_training  import (get_lr_scheduler, set_optimizer_lr,
                         weights_init)
from models.utils.callbacks import LossHistory, EvalCallback
from models.utils.dataloader import DeeplabDataset, deeplab_dataset_collate
from model.models.utils.utils import download_weights, show_config
from models.utils.utils_fit import fit_one_epoch
...
if __name__ == "__main__":
    ...
    #   所使用的的主干网络：
    #   mobilenet
    #   xception hrnetv2_48 resnest50 se_resnet101_ibn_a resnext50_ibn_a
    #---------------------------------#
    backbone        = "resnest50"
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    VOCdevkit_path  = r'C:\Users\wang\Desktop\VOCdevkit_pot'
    in_channels=3
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    #model=Deeplabv3plus_res101(num_classes=num_classes,downsample_factor=downsample_factor,pretrained=pretrained)
    # model   = deeplabv3plus_resnet18(num_classes=num_classes,output_stride=downsample_factor,pretrained_backbone=pretrained)

    # model = DeepV3Plus(in_channels=in_channels, n_classes=num_classes, backbone=backbone,pretrained=pretrained)
    model = DeepV3Plus_CA(in_channels=in_channels, n_classes=num_classes, backbone=backbone,pretrained=pretrained)
    print(model)
    # model = HRNet(in_channels=in_channels, n_classes=num_classes, backbone=backbone,pretrained=pretrained)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
