...
class HRNet_Backbone(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x, the_four_features = self.backbone(inputs)
        ...
        low_level_channels = F.interpolate(
            input=x[1], size=(x0_h, x0_w), mode="bilinear", align_corners=True
        )  # x1(B, 64, H/4, W/4)
        the_three_channels= F.interpolate(
        ...
        # ******************** Concat feature maps ********************
        x = torch.cat(tensors=[x[0], low_level_channels, the_three_channels, x3], dim=1)  # x(B, 480, H/4, W/4)
        # print("the_four_features channels:", the_four_features.size(1))
        # print("low_level_channels channels:", low_level_channels.size(1))
        # print("the_three_channels channels:", the_three_channels.size(1))
        # # print("the_three_features channels:", the_three_features.size(1))
        # print("x channels:", x.size(1))
        # # ******************** Fusion Convolution (main branch) ********************
        x = self.last_layer(x)  # x(B, last_inp_channels, H/4, W/4)
        return low_level_channels,the_three_channels,the_four_features, x
    ...
    #         x, c1 = self.backbone(inputs)
    #
    #         print(f"c1 : {c1.shape[1]} channels")
    #
    #         # Upsampling
    ...
    #
    #         print(f"x1 : {x1.shape[1]} channels")
    #         print(f"x2 : {x2.shape[1]} channels")
    #         print(f"x3 : {x3.shape[1]} channels")
    #
    #         # Concatenate
    #         x = torch.cat([x[0], x1, x2, x3], dim=1)
    #
    #         # Fusion Convolution
    #         c4 = self.last_layer(x)
    #
    #         print(f"c4 : {c4.shape[1]} channels")
    #
    #         return c1, c4
if __name__ == '__main__':
    model = HRNet_Backbone(backbone="hrnetv2_w32", pretrained=False)
    print(model)
    input_tensor = torch.randn(1, 3, 512, 512)  # Example input
    c1, c4 = model.forward(input_tensor)
